...
module.exports = function(){
    this.getAll =  async function(result){
        ...
        var sqlString = "SELECT * FROM Subject";
    ...
    this.create= async function(newData, result) {
        const pool = await conn;
        var sqlString =
          "INSERT INTO dbo.Subject(id, classHour, name) VALUES(@id, @classHour, @name)";
        return await pool
          .request()
          .input("id", sql.VarChar, newData.id)
          .input("classHour", sql.Int, newData.classHour)
    ...
    this.update= async function(newData, result) {
        const pool = await conn;
        var sqlString =
        "UPDATE dbo.Subject SET classHour = @classHour," +
        " name = @name " +
        " WHERE id= @id";
        ...
        return await pool
          ...
          .query(sqlString, function (err, data) {
            if (err) {
                ...
                result(null, "Delete subject success");
              }
          });
    }
}